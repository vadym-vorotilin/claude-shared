...
| "The command takes ten minutes, so a longer poll bound saves turns" | Each extra poll is one cache read. One bound past the cache lifetime is a full write — about ten reads' worth — so the longer bound costs more than every poll it saved, and it did: a fixer briefed with the 230-second form stretched it toward the Bash ceiling and expired its cache twice in a row. The bound is a maximum, never an example. |
